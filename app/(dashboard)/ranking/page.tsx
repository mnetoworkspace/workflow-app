      ...
      <div className="neon-card rounded-2xl overflow-hidden border border-[#ffe600]/10 mb-5">
        ...
        <div className="divide-y divide-white/[0.04]">
          {rankingSemanal.map((profile, index) => {
            ...
            return (
              <div key={profile.id} className={cn('flex items-center gap-3 px-4 py-3.5 transition-colors', isUser && 'bg-white/[0.03]')}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    {isUser && <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#00f0ff]/10 text-[#00f0ff] border border-[#00f0ff]/20 shrink-0">você</span>}
                  </div>
                  <span className="text-[11px] text-[#444466]">{titles[index] ?? '...'}</span>
                </div>
      ...
      <div className="neon-card rounded-2xl overflow-hidden border border-white/[0.07]">
        ...
        <div className="divide-y divide-white/[0.04]">
          {(profiles ?? []).map((profile, index) => {
            ...
            return (
              <div key={profile.id} className={cn('flex items-center gap-3 px-4 py-3.5', isUser && 'bg-white/[0.03]')}>
                <span className="text-xs text-[#333355] font-mono w-5 text-center shrink-0">{index + 1}</span>
                <Avatar className={cn('h-9 w-9 rounded-full shrink-0', frameClass(profile.avatar_frame))}>
                  {profile.avatar_url && <AvatarImage src={profile.avatar_url} alt={profile.name} />}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5 flex-wrap">
                    ...
                    {userBadges.slice(0, 3).map(badge => (
                      <span key={badge.id} className="text-xs leading-none" title={BADGE_INFO[badge.tipo as keyof typeof BADGE_INFO]?.label}>
